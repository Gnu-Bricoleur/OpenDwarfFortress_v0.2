...
#		picklableMethod = MethodProxy(someObj, someObj.method)
	...
	def save(self):
		pickle.dump( modifsasauvegarder, open( "Sauvegardes/seauve.p", "wb" ) )
		mon_fichier = open("seed.txt", "w")
	...
	def charge(self):
		dico=pickle.load(file("Sauvegardes/seauve.p"))
		mon_fichier = open("seed.txt", "r")
...
class Window(pyglet.window.Window):
	...
	def on_key_press(self, symbol, modifiers):
		...
		if symbol == key.Z:
			...
			nomcapture='Captures/'+str(graine)+'-'+str(time.time())+'.png'
...
#			cpickle.dump( picklableMethod, open( "Sauvegardes/saveWorld.p", "wb" ) )
			...
			if enregistrementdemodele == False:
				...
				enregistrementdemodele = False
				positionpremierbloc=(0,0,0)
				pickle.dump( modeleenenregistrement, open( "Mesmodeles/modele.p", "wb" ) )
				modeleenenregistrement={}
...
def menu():
	...
	pygame.display.set_caption("ODF v0.2")
